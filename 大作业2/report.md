...
1. 计算$t(a)$时的误差被带入进了$t_k$的计算中,下逐项分析辛普生积分公式中包含的舍入误差；
2. 在进行积分计算过程中每一步计算后进行舍入造成的误差。

因为上述共有两个独立的舍入误差来源且第一个误差来源较为复杂，所以我们希望选取合适的计算过程中舍入精度使其造成尽可能的影响。
...
而每一项涉及到$e$指数次幂计算的过程都会出现舍入误差，可以认为计算过程中每一步保留的小数位数均为$m_1$位，则由此引入的舍入误差为：
$$
\delta = \frac{1}{2}\times 10 ^{-m_1}
$$
综上，本题中利用积分公式计算时引入的舍入总误差如下所示：
$$
\begin{align}
R_r[f] &= \frac{1}{6}\cdot(\delta[0] + \delta[t(a)^2e^{t(a)^2}]) + \frac{1}{3}\cdot n \cdot \delta [\alpha_{1k}] + \frac{2}{3} \cdot n\cdot \delta[\alpha_{2k}] + \frac{1}{2}\times 10 ^{-m_1} \times 2n\\
&\leq (\frac{7}{2}\cdot t(a)^2 + \frac{7}{3}t(a)^4)\cdot e^{t(a)^2}\cdot \delta[t(a)] + n \times 10^{-m_1}\\
\end{align}
$$
为了避免系统欠定，也为了便于进行误差分析，不妨假设$m_1 == m$，则有：
$$
R_r[f] \leq [(\frac{7}{2}\cdot t(a)^2 + \frac{7}{3}t(a)^4)\cdot e^{t(a)^2} + 2n]\cdot  \frac{1}{2}\times 10^{-m}\\
$$
...
R_r[f] & \leq [(\frac{7}{2}\cdot t(a)^2 + \frac{7}{3}t(a)^4)\cdot e^{t(a)^2} + 2n]\cdot  \frac{1}{2}\times 10^{-m}& = \frac{1}{4}\times 10^{-x}\\
\end{align}
\right.
$$
...
[(\frac{7}{2}\cdot t(a)^2 + \frac{7}{3}t(a)^4)\cdot e^{t(a)^2} + 2n]\cdot 2\times 10^{x} &\leq 10^{m}\\
\end{align}
\right.
$$
...
2. 当$a = 1,x = 6$时，积分结果为：0.550832

   <img src="report.assets/image-20211214141747051.png" alt="image-20211214141747051" style="zoom:80%;" />
...
`gmpy2`库中提供的精度设置函数只能够设置精确到多少位的二进制数，因此会导致一个数的整数部分干扰小数部分的精确程度。因此对于第一问的计算过程来说，应当尽可能地避免整数部分的长度对整个数据的精度产生干扰。因此需要在设置计算精度之前预知整数部分的长度，将设置的精度调整为计算需要的精度+整数部分的长度。因此我在第一问的计算过程中采用了如下的算法：

首先以较低精度计算结果的近似值，以确定计算过程中整数部分的最大长度，再利用该最大长度确定计算过程中应当设置的精确位数，以确保计算的精度一定能够满足误差要求。

###### 第三问中的精度实现

为了控制第三问计算结果中的舍入误差，我将第一问完成的计算过程进行封装，使其能够返回一个满足给定精度要求的有理数形式，这样就可以在第三问（涉及到误差传递）的问题中获得满足精度要求的数值，同时在后续的计算中尽可能地不引入更多的舍入误差，以便于进行误差分析。

而第三问比较棘手的是舍入误差会有两个不同的来源，正如上文提到的，首先是来源于解微分方程得到的积分上限，而这个误差会随着计算$h$而进入到每一项之中；而再计算的过程中，也会由于近似的舍入造成每一步的误差，但由于积分的计算公式是求和式，且误差来源于计算过程的最后一步，可以近似将每个求和式的该项误差求和即可。同时为了分析的便利，可以假设计算过程中的保留精度相同。因而我在第三问的实现过程中，每一个子式的计算都将指数求幂的运算放到了尽可能靠后的位置以便尽可能晚的引入误差，以更加精准的贴合计算结果。
